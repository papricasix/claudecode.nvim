...
- Under Changes, split at `layout.sessions_height` so its divider lines up with Sessions/Activity; `state.sizes.changes` is snapshotted and restored like the other sizes. Draws the selected session's subagents as a tree: connectors, state glyph (`●` running, `✓` done, `✗` failed, `⊘` stopped), name, tokens, runtime.
- The name column shows each run's `description` by default (several `general-purpose` rows are told apart only by it); `g.` (`keymaps.subagent_label`) toggles to the agent type (`model.subagent_label`, lives on the view and dies with `detach`, like the Activity filter). The winbar marks only the non-default reading (`Subagents` / `Subagents · type`) instead of a notify. A description is collapsed to one line and falls back to the type when empty. The name's room floors at **1 cell**, not a readable minimum, so it is always cut with `…` rather than pushing the numbers past the window edge, where Neovim clips silently.
...
- **`<CR>` opens the run's transcript in a float** (`agents/subagent_view.lua`, routed through `open_row` like tool rows, so `<C-n>`/`<C-p>` stepping works; `next_open_row` counts `kind = "subagent"` rows). A pure core (`fold_line` → document, `render` → Markdown lines, marks, links, folds) is what the specs drive; `filetype = markdown`, so render-markdown/markview draw it.
  ...
  - Thinking is a closed manual fold per block (`_thinking_` header). Live updates rewrite only from the **first line that changed**, so folds above it keep the reader's open/closed state, and a cursor on the last line follows the end.
  - User entries: the first is `## Prompt`; bracketed/tagged ones are the harness (`> …`); an interrupt is `> ⊘ interrupted`; a `<task-notification>` is a `finished:` line. A call with no result while the run is not running reads `⊘`.
