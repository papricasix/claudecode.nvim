...
- `g.` (`keymaps.subagent_label`) toggles the name column between agent type and `description` (`model.subagent_label`, lives on the view and dies with `detach`, like the Activity filter). The winbar says which (`Subagents` / `Subagents · what for`) instead of a notify. A description is collapsed to one line and falls back to the type when empty. The name's room floors at **1 cell**, not a readable minimum, so it is always cut with `…` rather than pushing the numbers past the window edge, where Neovim clips silently.
