...
- **Ends nothing notifies**: a shell or monitor stopped with `TaskStop` gets **no notification** — its only transcript record is the TaskStop result `{message = "Successfully stopped task: …", task_id, task_type = "local_bash"}` (`note_task_stop` → `sum.task_stops`). The CLI also closes every task's output file with `\n[exited with code N]\n` or `\n[killed]\n` (stopped, or expired); `subagents.output_footer` reads the last 128 bytes (`_io.read_tail_sync`, cached by size/mtime) as the backstop. `shell_state` precedence: status notification → TaskStop → expiry event → footer → file gone (stopped) → live/recent (running). `how` = `killed`/`expired`.
- The fold's table is `sum.tasks` (shells and monitors) with pending calls in `sum.task_calls`; rows use `kind = "shell"` for both (they share `shell_view`).
