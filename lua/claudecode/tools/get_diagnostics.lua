...
-- The wire shape is VS Code's: one text block holding a list of files, each with
-- its URI and its diagnostics, and each diagnostic carrying an LSP range (0-based
-- lines and characters) and a severity *name*. The previous shape -- one text
-- block per diagnostic, 1-indexed positions and a numeric severity -- was ours
-- alone, and Claude reads the VS Code one.
...
---A diagnostic knows its buffer; a buffer with no name (a scratch one) falls back
---to the path the request named, if it named one.
